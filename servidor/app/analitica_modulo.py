...
class analitica():
    ...
    def operaciones(self, sensor):
        ...
        if ("max-{}".format(sensor)=="max-humedad".format(sensor)) and str(df_filtrado.max(skipna = True))>"70":
            self.publicar("alerta-humedad".format(sensor), "ALERTA Humedad Alta")

        if ("min-{}".format(sensor)=="min-humedad".format(sensor)) and str(df_filtrado.min(skipna = True))<"60":
            self.publicar("alerta-humedad".format(sensor), "ALERTA Humedad Baja")

        if ("max-{}".format(sensor)=="max-temperatura".format(sensor)) and str(df_filtrado.max(skipna = True))>"30":
            self.publicar("alerta-temperatura".format(sensor), "ALERTA temperatura Alta ")

        if ("min-{}".format(sensor)=="min-temperatura".format(sensor)) and str(df_filtrado.min(skipna = True))<"23":
            self.publicar("alerta-temperatura".format(sensor), "ALERTA temperatura Baja")

        if ("max-{}".format(sensor)=="max-lum".format(sensor)) and str(df_filtrado.max(skipna = True))>"60":
            self.publicar("alerta-lum".format(sensor), "ALERTA Iluminacion Alta ")

        if ("min-{}".format(sensor)=="min-lum".format(sensor)) and str(df_filtrado.min(skipna = True))<"30":
            self.publicar("alerta-lum".format(sensor), "ALERTA Iluminacion Baja")
    ...
    def regresion(self, sensor):
        ...
        for tiempo, prediccion in zip(nuevos_tiempos, Y_pred):
            ...
            self.publicar("dato_futuro-{}".format(sensor), "{:.2f}".format(prediccion[0]))
    @staticmethod
